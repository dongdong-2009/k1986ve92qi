...
void PortConfig()
{
	...
	MDR_PORTA->FUNC = 0x0000;				  	/* mode is port */
	MDR_PORTA->RXTX = 0x0000;			     	/* clear the out */
	MDR_PORTA->OE = 0xff;					/* port is output mode */
	MDR_PORTA->ANALOG = 0xff;				/* port is digital mode */
	MDR_PORTA->PWR |= 0xffff;				/* max power of port */

	MDR_RST_CLK->PER_CLOCK |= 1<<22;	 	//clock of PORTB ON	
	MDR_PORTB->FUNC |= (3<<(9<<1));
	MDR_PORTB->OE = 0;						/* input mode */
	MDR_PORTB->ANALOG |= (1<<9);			/* digital mode */
